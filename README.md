...
Bun 1.x, TypeScript estrito, NestJS, PostgreSQL já vem com ela local ou ser preferir pode criar a URL database(hospedado no
[Neon](https://neon.tech)) nesse caso so trocar a URL .env, SQS via LocalStack, MikroORM, Docker Compose.
## Banco de dados: container local ou Neon (Postgres hospedado)

Este projeto usa Postgres local se quiser alterar pra hospedado pode também.
